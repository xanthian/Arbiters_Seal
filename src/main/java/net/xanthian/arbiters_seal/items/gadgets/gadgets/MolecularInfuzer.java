...
public class MolecularInfuzer extends ModGadgetItem {

    @Override
    public ActionResult useOnEntity(ItemStack stack, PlayerEntity player, LivingEntity entity, Hand hand) {
        if (!player.getWorld().isClient && player.getEquippedStack(EquipmentSlot.MAINHAND).isOf(this)) {
            entity.addStatusEffect(new StatusEffectInstance(StatusEffects.STRENGTH, 2000, 0,false, false, true),player);
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity player, Hand hand) {
        if (Screen.hasShiftDown()) {
            if (!player.getWorld().isClient && player.getEquippedStack(EquipmentSlot.MAINHAND).isOf(this)) {
                ...
                player.addStatusEffect(new StatusEffectInstance(StatusEffects.STRENGTH, 2000, 0,false, true, true),player);
                if (!player.getAbilities().creativeMode) {
                    stack.setDamage(stack.getDamage() + 1);
                    if (stack.getDamage() > stack.getMaxDamage()) {
                        stack.setCount(0);
